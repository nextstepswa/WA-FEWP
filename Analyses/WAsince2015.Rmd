...
Here we report the annual totals up through the last complete year --  `r last_complete_yr` -- with data from the MPV project only, since it provides the only consistent data series.  This limits the cases to those meeting the MPV inclusion criteria (excludes suicides and most pursuit related fatalities).
...
states.all <- bind_rows(rates.tmp, notwa) %>%
  ...
  mutate(across(starts_with('shot'), 
                ~ 1000000 * ./get(str_replace(cur_column(), 'shot', 'POPESTIMATE')), 
                .names = 'rate{.col}'))
...
state.rates <- states.all %>%
  ...
  pivot_longer(contains("rate"),
               names_to = "year") %>%
  mutate(year = as.numeric(sub("rateshot", "", year))) %>%
  rename("rate" = "value")
...
* These are interactive plots:  You can hover over bars to get the exact values for the number of persons shot in `r last_complete_yr`.
...
* These are interactive plots:  You can hover over bars to get the exact values for the rate of persons shot in `r last_complete_yr`.
...
* These are interactive plots:  You can hover over bars to get the exact values for the number of persons shot in `r last_complete_yr` year, and the percent change from the previous year.
