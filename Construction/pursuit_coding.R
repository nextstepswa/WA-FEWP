...
# input df: pursuit_draft_2015
# output files: csv files with cases from pursuit_draft_2015 to review; WTSC RDA file
...
all.pursuit.tags <- pursuit_draft_2015 %>%
  ...
  mutate(pursuit.tag = case_when(
    ...
    grepl('vehicle|car|crash|speed|chase|pursuit|flee|fled', description) |
      grepl('Vehicle|Other', fleeing) ~ 2, # any other indication of vehicle or pursuit
...
## Replace NA in vpursuit.draft with a reminder to review

old.coded <- raw.coded.pursuits %>%
  select(feID, date)

all.cases <- pursuit_draft_2015 %>%
...
if(lubridate::month(Sys.Date()) > 5 & 
  ...
  message(paste("\n\n *** CHECK FOR WTSC ANNUAL UPDATE (last yr in dataset is",
                last.wtsc.update, 
                ") **** \n\n\n"))
  wtsc.update.message <- "WTSC data update needed"
