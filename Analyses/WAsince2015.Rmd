...
`r last.name`, `r last.age`, on `r format(last.date, format="%B %d, %Y")` by `r last.agency`__
    ...
    `r ifelse(last.name == "(Name not released)", "This", last.name)` is the `r paste0(tot.this.yr, num.suffix)` person killed by police in `r tot.yr.is`.  The cause of death is reported as `r str_to_lower(last.cod)` and the manner of death appears to be `r str_to_lower(last.mod)`.  `r ifelse(grepl("Not|accident", last.pursuit), "", "This incident involved a police vehicular pursuit.")`  More information can be found in this article: `r last.url`
...
Clickable urls are available in this report in the [Interactive Map](#interactive-map) and [Say their names](#say-their-names) sections. When clicked, they will take you to the article.
...
Here we report the annual totals up through the last complete year --  `r last_complete_yr` -- with data from the MPV project only, since MPV provides the only consistent national data series.  This limits incidents to those meeting the MPV inclusion criteria, which includes people killed by police with firearms, tasers, asphyxiation and other uses of force, but excludes most pursuit related fatalities.

## Compared to all other states {.tabset}


```{r wavsus, warning=F, include=F}
# Here we need the complete datasets, not just WA state

# Note: current year values are hardcoded here and need to be updated on Jan 1 of each year.

mpv_tab <- mpv_clean_2015 %>% 
    filter(year >= start_yr & year <= last_complete_yr) %>%
  #filter(match(month, month.abb) <= last_mpv_date - 1) %>%
  mutate(wa = ifelse(st == "WA", "WA", "All other states")) %>%
  group_by(year, wa) %>%
  count() %>%
  pivot_wider(names_from = wa,
              values_from = n) %>%
  ungroup() %>%
  mutate(pct.change.other = scales::percent((`All other states`/lag(`All other states`) - 1)),
         pct.change.wa = scales::percent((WA/lag(WA) - 1)))


# Note the number of regions has changed between the decades
# Also the estimates for 2020 are different, so we take the average
# As of 4/11/2025 these files can no longer be scraped using https://www2.census.gov/programs-surveys/popest/datasets/ 
# but can be scraped with "ftp://ftp2..."
# May still need to download in the future as so much govt data access is uncertain now

# Pop estimates for 2025 are not yet available (as of 1/23/2026) so we estimate 2025 based on average growth rate
# from 2020-2024

pop2020 <- read_csv("ftp://ftp2.census.gov/programs-surveys/popest/datasets/2010-2020/state/totals/nst-est2020-alldata.csv") %>%
  filter(SUMLEV != "020") %>%
  select(NAME, contains("POPESTIMATE"))
  
pop2025 <- read_csv("ftp://ftp2.census.gov/programs-surveys/popest/datasets/2020-2024/state/totals/NST-EST2024-ALLDATA.csv") %>%
  filter(SUMLEV %in% c("010", "040")) %>%
  select(NAME, contains("POPESTIMATE")) %>%
  mutate(POPESTIMATE2025 = POPESTIMATE2024 + POPESTIMATE2024 * (POPESTIMATE2024-POPESTIMATE2020)/(POPESTIMATE2020*4))


pop <- left_join(pop2020, pop2025, by = "NAME") %>%
  rowwise() %>%
  mutate(POPESTIMATE2020 = mean(c_across(contains("2020")))) %>%
  select(NAME, POPESTIMATE2015:POPESTIMATE2019, POPESTIMATE2020, POPESTIMATE2021:POPESTIMATE2025) %>%
  filter(NAME != "Puerto Rico") %>%
  rename("state" = "NAME")


rates.tmp <- mpv_clean_2015 %>% 
  filter(year >= start_yr & year <= last_complete_yr) %>%
  ...
  bind_rows(data.frame(c(pop[1,1], bind_rows(.[,-1] %>% map_dbl(., sum, na.rm=T))))) %>%
  left_join(pop)

notwa <- data.frame(state="All Other States", 
                    subset(rates.tmp, state=="United States")[-1] - subset(rates.tmp, state=="Washington")[-1])

# using rates per million
states.all <- bind_rows(rates.tmp, notwa) %>%
  # h/t https://stackoverflow.com/questions/69831615/dividing-a-set-of-columns-by-another-set-of-columns-based-on-their-column-names
  mutate(across(starts_with('killed'), 
                ~ 1000000 * ./get(str_replace(cur_column(), 'killed', 'POPESTIMATE')), 
                .names = 'rate{.col}'))

state.rates <- states.all %>%
  select(state, contains("rate")) %>%
  pivot_longer(contains("rate"),
               names_to = "year") %>%
  mutate(year = as.numeric(sub("ratekilled", "", year))) %>%
  rename("rate" = "value")
...
Keep in mind that this uses the MPV data, so it excludes most people killed in police pursuits.

```{r wavsus-rates, warning=F}
...
## State rankings `r last_complete_yr` {.tabset}

Here we report the MPV data by state for the most recent complete year: `r last_complete_yr`.


### Number killed

* States are ranked by the the number of persons killed in `r last_complete_yr`.

* The bar for WA State is highlighted with a black border.

* These are interactive plots:  You can hover over bars to get the exact values for the number of persons killed in `r last_complete_yr`.

```{r strank-number, warning=F, fig.height=7}

p <- ggplot(data = states.all %>% filter(!grepl("United|Other", state)),
            aes(x=killed2025, y=reorder(state, killed2025), 
                fill = killed2025,
  ...
  scale_fill_gradient2(
    ...
    mid = "white", 
    high = "red", 
    midpoint = median(states.all$killed2025)) +
...
* States are ranked by the population adjusted rate of persons killws in `r last_complete_yr`:  the rate per million state residents.

* The bar for WA State is highlighted with a black border.

* These are interactive plots:  You can hover over bars to get the exact values for the rate of persons killed in `r last_complete_yr`.


```{r strank-rate, warning=F, fig.height=7}

p <- ggplot(data = states.all %>% filter(!grepl("United|Other", state)),
            aes(x=ratekilled2025, y=reorder(state, ratekilled2025), 
                fill = ratekilled2025,
                color = factor(ifelse(state=="Washington", 1, 0)),
                text=paste(state, 
                           "total:", round(ratekilled2025, 1)))
            ) +
  ...
  scale_fill_gradient2(
    ...
    mid = "white", 
    high = "red", 
    midpoint = median(states.all$ratekilled2025)) +
  #scale_x_continuous(labels = scales::percent) +
...
* States are ranked by the percent change in the number of persons killed from `r last_complete_yr-1` to `r last_complete_yr`.

* The bar for WA State is highlighted with a black border.

* These are interactive plots:  You can hover over bars to get the exact values for the number of persons killed in `r last_complete_yr` year, and the percent change from the previous year.

```{r strank-change, warning=F, fig.height=7}
df <- states.all %>% 
  filter(!grepl("United|Other", state)) %>%
  select(state, killed2023:killed2025) %>%
  mutate(pct.chg = killed2025/killed2023 -1)
...
p <- ggplot(data = df,
            aes(x=pct.chg, y=reorder(state, pct.chg), 
                ...
                color = factor(ifelse(state=="Washington", 1, 0)),
                text=paste0(state, 
                           paste("<br>", "number killed in ", last_complete_yr, ": ", killed2025),
...
* In 2023 [SB 5352](https://lawfilesext.leg.wa.gov/biennium/2023-24/Pdf/Bills/Session%20Laws/Senate/5352.SL.pdf?q=20231220183138) modified the [policy on vehicular pursuits](https://app.leg.wa.gov/RCW/default.aspx?cite=10.116.060), reducing the standard of evidence from probable cause to reasonable suspicion, adding some offenses to the list of pursuit-eligible offenses, and relaxing the supervisory authorization requirements.

* In 2024 [Initiative 2113](https://www2.sos.wa.gov/_assets/elections/initiatives/finaltext_3034.pdf) further modified the statewide pursuit policy, eliminating the specific list of offenses that restricted pursuits to violent felonies, and allowing them for any suspected criminal offense.
